...
pub fn push_reload_ron(cargo_pkg_name: &str) {
	...
	let add = String::from(cargo_pkg_name);
	if !reloads.contains(&add) {
		reloads.push(add);
	}

	let mut write = String::from("//This file is written to and managed automatically. Running `cargo build` on a scrap will add it here, so long as you are implementing engine-build functionality within that scrap.\n");
